...
switch ($action)
{
    case 'modules':
        ...
        {
            $mod = $_GET['module'];
            if (!filter_var($mod, FILTER_VALIDATE_REGEXP, array('options' => array('regexp' =>
                        '/^[a-zA-Z0-9\_]+$/'))))
                Func::redirect('/panel/dashboard/1');
            if (!class_exists($mod) || !file_exists(ROOTPATH . '/iwbx-includes/modules/' . $mod .
                '.php'))
            ...
            if (count($modules))
            {
                ...
                foreach ($modules as $module_file)
                {
                    ...
                    echo '<a class="list-group-item" href="' . $baseurl . '/' . $controller .
                        '/modules/module/' . $module . '">' . $module::getName() . '</a>';
                }
                echo '</div>';
            }
    ...
    case 'dashboard':
        ...
        echo '<div class="col-sm-6"><div><dl><dt>URL</dt><dd><a href="//' . $site['url'] .
            '">http://' . $site['url'] . '</a></dd><dt>Mendaftar</dt><dd>' . Func::
            displayDate($site['time']) . '</dd><dt>Expired</dt><dd>Never</dd></dl></div>';
        echo '</div></div>';
        break;
    ...
    case 'delete_site':
        ...
        echo '<ol class="breadcrumb"><li><a href="' . $baseurl . '">' .
            '<i class="fa fa-home"></i> Home</a></li>' . '<li><a href="' . $baseurl .
            '/panel">' . 'Panel</a></li>' . '<li class="active">Hapus Situs</li></ol>';
        echo '<div class="form"><form method="post" action"' . $baseurl . '/' . $controller .
            '/' . $action . '"><div class="alert alert-warning">Kamu yakin akan menghapus situs <strong>' .
            $site['url'] . '</strong>?</div><p><button class="btn btn-primary btn-sm" type="submit" name="submit">Ya Hapus</button>' .
    ...
    case 'create_site':
        ...
        if ($req->rowCount() >= $set['maxsites'])
        {
            ...
            echo '<div class="alert alert-danger">Anda sudah tidak diijinkan lagi membuat situs baru,' .
                ' Maksimal jumlah situs per user adalah ' . $set['maxsites'] . '.</div>';
        ...
        if (isset($_POST['submit']))
        {
            ...
            elseif (!filter_var($subdomain, FILTER_VALIDATE_REGEXP, array('options' => array
                    ('regexp' => '/([a-z0-9-]{4,16}+$)/'))))
                $errors['subdomain'] =
                    'Subdomain hanya diperbolehkan karakter a-z, 0-9 dan simbol -.';
            elseif (mb_substr($subdomain, 0, 1) == '-' || mb_substr($subdomain, -1) == '-')
                $errors['subdomain'] = 'Subdomain tidak boleh diawali atau diakhiri simbol -.';
            ...
            else
            {
                ...
                if ($req->rowCount() > 0)
                    $errors['subdomain'] = 'Alamat situs <strong>' . $subdomain . '.' . $domain .
                        '</strong> sudah terdaftar.';
            }
            if (empty($errors))
            {
                if (mkdir(ROOTPATH . 'iwbx-sites/' . $subdomain . '.' . $domain, 0777))
                {
                ...
                {
                    $errors['subdomain'] = 'Pembuatan situs gagal. Silakan hubungi Administrator';
                }
        ...
        echo '<ol class="breadcrumb"><li><a href="' . $baseurl . '">' .
            '<i class="fa fa-home"></i> Home</a></li>' . '<li><a href="' . $baseurl .
            '/panel">' . 'Panel</a></li>' . '<li class="active">Membuat Situs</li></ol>';
        if ($errors)
    ...
    case 'index':
        ...
        if ($res->rowCount() > 0)
        {
            ...
            {
                echo '<li class="list-group-item"><h4 class="list-group-item-heading"><a href="//' .
                    $site['url'] . '" target="_new"><strong><i class="fa fa-globe"></i> ' . $site['url'] .
                    '</strong></a></h4><div class="list-group-item-text"><ul><li>Dibuat: ' . Func::
                    displayDate($site['time']) . '</li></ul><div style="margin-top:10px;"><a href="' .
                    $baseurl . '/panel/switch/id/' . $site['site_id'] .
                    '"><i class="fa fa-sign-in"></i> Kelola</a>&nbsp;&nbsp;&nbsp;<a href="' . $baseurl .
                    '/panel/delete_site/id/' . $site['site_id'] .
