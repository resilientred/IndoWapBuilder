...
switch ($action)
{
    ...
    case 'whois':
        ...
        echo '<div class="col-sm-offset-4"><form method="get" action="' . $baseurl .
            '/site/whois">' . '<div class="input-group"><input class="form-control input-sm" type="text" name="domain" value="" ' .
        ...
        if (isset($_GET['domain']))
        {
            ...
            {
                $site = $br->fetch();
                $uz = Base::db()->query("SELECT `name` FROM `user` WHERE `user_id` = '" . $site['user_id'] .
                    "'");
                $mass1 = $uz->fetch();
                echo '<div class="well well-sm"><dl class="dl-horizontal">' .
                    '<dt>URL Situs</dt><dd><a href="http://' . $domain . '">http://' . $domain .
                    '</a></dd><dt>Pemilik</dt><dd>' . htmlspecialchars($mass1['name']) .
        ...
        if ($subdomain && $domains && is_array($domains))
        {
            ...
            foreach ($domains as $domain)
            {
                if (in_array($domain, $site_domains))
                {
                    ...
                    echo '<div style="background-color:#f9f9f9;box-shadow: 0 3px 9px #ccc;padding:10px;">';
                    if ($total[0] == 0)
                    {
                        echo '<div class="alert alert-success" style="margin-bottom:5px"><h4>' . $url .
                            '</h4><p>Domain masih tersedia!</p>' .
                            '</div><p><a class="btn btn-primary btn-block btn-sm" href="' . $baseurl .
                            '/panel/create_site/domain/' . $url . '">Pendaftaran</a></p>';
                    }
                    else
                    {
                        echo '<div class="alert alert-danger" style="margin-bottom:5px"><h4>' . $url .
                            '</h4><p>' . 'Domain tidak tersedia!</p>' .
                            '</div><p><a class="btn btn-primary btn-block btn-sm" href="' . $baseurl .
                            '/site/whois/domain/' . $url . '">WHOIS</a></p>';
                    }
                    echo '</div>';
                    echo '</div>';
                }
    ...
    case 'forgot_password':
        ...
        echo '<div class="form"><form role="form" action="' . $baseurl .
            ...
            '</div>' . '<p><button class="btn btn-primary btn-sm" type="submit" name="submit">Kirim</button></p>' .
            '</form></div>';
        break;
    ...
    case 'login':
        ...
        if ($redirect)
        {
            if (!filter_var($redirect, FILTER_VALIDATE_URL, FILTER_FLAG_SCHEME_REQUIRED))
                $redirect = false;
        }
        if (isset($_POST['submit']))
        {
        ...
        echo Func::getNotice() . '<div class="form"><form role="form" action="' . $baseurl .
            '/site/login' . ($redirect ? '?redirect=' . $redirect : '') . '" method="post">' .
    ...
    case 'index':
        ...
        echo '<h3 class="text-center">Buat wap site gratis</h3>' .
            '<p style="margin-bottom:30px;" class="text-center">' . htmlspecialchars($set['sitename']) .
